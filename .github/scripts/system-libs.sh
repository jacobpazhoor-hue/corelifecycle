...
# SIGTERM is not honoured.
APT=(-o Acquire::Retries=2 -o Acquire::http::Timeout=15 -o Acquire::https::Timeout=15)

sudo timeout -k 10 120 apt-get "${APT[@]}" update \
  || echo "::warning::apt-get update stalled or failed; continuing with the runner image's preinstalled packages"

sudo timeout -k 10 240 apt-get "${APT[@]}" install -y --no-install-recommends "${EXTRA[@]}" "${PKGS[@]}" \
  || sudo timeout -k 10 240 apt-get "${APT[@]}" install -y --no-install-recommends "${EXTRA[@]}" libasound2 \
...
for so in libnss3.so libdbus-1.so.3 libatk-1.0.so.0 libatk-bridge-2.0.so.0 libcups.so.2 \
  ...
  ldconfig -p | grep -q "$so" || missing="$missing $so"
done
for bin in "${EXTRA[@]}"; do
