...
| 4 | **✅ DONE 2026-07-04** — Camera 2.0: level-cut IMPACT (whip-in + decaying shake + 1-2f flash, synced to the stamp/thud SFX) + foreground OCCLUDER (parallax depth, no template refactor). Validated no cost regression, gate PASS. Commit 9802cfe. Still deferred (need per-template layer refactor or event signals): full 3-layer BG parallax, dolly-zoom on midpoint reversal, smears → fold into step 7. | src/Video2.tsx (anim.ts) | done |
| 5 | **✅ DONE 2026-07-04** — Format extensions: topic_queue.json refreshed (18 demand-proven topics w/ `format`+`proof`), AUTOPILOT_PROMPT+BIBLE format handling (rank/history/scenario/survival ladders + title spines + label vocab + thumb archetype), Video2 grade now by level-start ORDINAL (byte-identical for LEVEL format, works for DAY/HOUR/RANK). gate+packaging format-agnostic. Thumbnails reuse existing archetypes (no new component). Commit 6cfd9e6. | ops/topic_queue.json, AUTOPILOT_PROMPT.txt, BIBLE.md, src/Video2.tsx | done |
| 6 | **✅ DONE 2026-07-04** — Growth ops: docs/WEEKLY_OPS.md (per-upload/weekly Studio checklist + KPI dashboard + cadence decision), gen_packaging emits pinned_comment + community_poll + binge tease, routine.json _cadence/kpi_targets. Cadence launchd change left to USER (daily stays). Commit 266bb82. | gen_packaging.py, docs/WEEKLY_OPS.md, ops/routine.json | done |
